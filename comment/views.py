    ...
    if request.method=='POST':
        ...
        if comment_form.is_valid():
            ...
            if not parent is None:#有回复
                ...
                # 新增代码,发送通知
                if not parent.user.is_superuser:#用户之间相互发送消息
                    if parent.user!=request.user:#排除发给自己
                        notify.send(
                            request.user,  # 发送者
            ...
            if not parent is None:  #
                if not request.user.is_superuser:#用户发给管理员的
                    notify.send(
                        request.user,
            ...
            data['comment_time']=localtime(comment.comment_time).strftime('%H:%M:%S')
