...
def page(request, element_list):
    context = {}
    paginator = Paginator(element_list,8)
    # 获取参数页码
...
def week_readnum_list(context):
    ...
    return context
"""表单"""
def get_form_all(context):
    # 登录注册表单
    context['register_form'] = RegisterForm()
    context['login_form'] = LoginForm()
    return context
"""昨日将日本周热门博客列表"""
def week_yesterday_today_hotbloglist(context):
    blog_content_type = ContentType.objects.get_for_model(Blog)
...
def blog_with_type(request, blog_type_id):
    ...
    get_form_all(context)
    week_yesterday_today_hotbloglist(context)
    week_readnum_list(context)
